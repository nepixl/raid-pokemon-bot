...
if (QUEST_LOCATION == true) {
    // Send location.
    $loc = send_venue($chat, $quest['lat'], $quest['lon'], "", !empty($quest['address']) ? $quest['address'] . ', ID = ' . $quest['id'] : $quest['pokestop_name'] . ', ' . $quest['id']); // DO NOT REMOVE " ID = " --> NEEDED FOR CLEANUP PREPARATION!
